...
const getBookCountFromCache = () => {
  ...
  return hyper.cache.get('stats')
      .then(result => { 
        console.log(' hyper.cache.get(stats) result', 'result:', result )
        return result.ok === false ? Promise.reject(key) : propOr( null , "bookCount" , result)
      })
...
const incrementCacheBookCount = () => getBookCountFromCache()
  .then( currentBookCount => hyper.cache.set('stats', { bookCount: currentBookCount + 1}), passValueThru)
