...
const setBookCountToCache = (bookCount) => hyper.cache.set('stats', {
    bookCount
})
...
const getBookCountFromCache = (key) => {
    ...
    return hyper.cache.get(key)
        .then(result => result.ok === false ? Promise.reject(key) : propOr( null , "bookCount" , result))
